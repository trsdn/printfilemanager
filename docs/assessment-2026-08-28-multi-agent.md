...
| P2-6 | **Failing test with a broken skip guard.** `externalRealFixtureURL()` checks that the *pointer file* exists but never that the file it points to exists, so a deleted fixture fails instead of skipping. The pointer targeted a file in the developer's Downloads folder that no longer exists. No `.3mf` fixture is committed, so the only real-world integration test is unreproducible and un-CI-able. | `Quicklook/Tests/ThreeMFCoreTests/ThreeMFCoreTests.swift:92-111` |
